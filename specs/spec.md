...
- **FR-002**: The system MUST let a creator specify a meeting place using typed text, a map pin, a selected venue, live location, or a combination of these. The place model MUST support a moving or transient context such as a moving train or a queue. Before creator selection, recipients MUST see only an approximate place/distance band; the exact pin, venue address, or live/moving location MUST be disclosed only after the creator selects that recipient. **[NEEDS CLARIFICATION: define required fields and how a moving/transient location is updated and re-disclosed to already-selected recipients.]**
...
- **FR-004**: The system MUST use a recipient's registered city interests when determining whether an active offer is relevant to that recipient. A recipient MUST additionally be within a configurable distance/radius of the offer's location to be counted as an eligible recipient. **[NEEDS CLARIFICATION: define the default/configurable radius value(s) and whether they vary by city or population density.]**
- **FR-005**: When a recipient has multiple registered city interests, the system MUST prioritize offers associated with the recipient's current location over otherwise comparable offers for other registered city interests. Current location MUST be sampled via device GPS periodically while the app is open; when live GPS is unavailable, the system MUST fall back to the recipient's last known location. **[NEEDS CLARIFICATION: define the "nearby" radius/threshold, the GPS sampling interval, permission-denied behavior, and how long a "last known location" remains valid before it is treated as stale.]**
- **FR-006**: The system MUST present or notify eligible recipients of an offer only while the offer is active, via push notification targeting delivery within approximately 30 seconds of publish, with the in-app live feed as a fallback channel. **[NEEDS CLARIFICATION: define behavior when push permission is denied, and the measurement/alerting approach for the 30-second delivery target.]**
- **FR-007**: The system MUST allow an eligible recipient to express interest in an active offer.
- **FR-008**: The system MUST allow the creator to select which interested recipients join an offer, up to a capacity the creator sets at publish time. Capacity MUST be fixed once the offer is published and MUST NOT be changed while the offer is active. **[NEEDS CLARIFICATION: define the minimum/maximum capacity limits and whether the creator may revise which specific recipients are selected, distinct from the fixed capacity number, before the offer expires.]**
...
- **FR-012**: The system MUST retain a creator-visible history of past offers and MUST allow the creator to rebroadcast an expired offer. **[NEEDS CLARIFICATION: determine whether rebroadcast creates a new offer, what information is reused, and any rate/cap limits.]**
...
- **FR-015**: The system MUST provide v1 block/report, content-moderation, emergency-guidance, trusted-contact-sharing, and ratings/reviews capabilities. A block request MUST be routed to moderation review before it takes effect broadly (on visibility or contact between the two users), but the submitting user's own view MUST update immediately so they stop seeing the blocked user, independent of the pending review outcome. **[NEEDS CLARIFICATION: define the exact user journeys, review handling and turnaround time, privacy boundaries, service levels, and enforcement rules for content moderation, emergency guidance, trusted-contact sharing, and data retention.]**
...
- **FR-039**: The system MUST screen offer activity text via automated keyword/content screening at publish time to block obvious violations of the prohibited-activity rules in FR-016 (paid meetings, private-home offers, dating/romantic framing, and other disallowed activities). Human moderation review is NOT required before an offer publishes in v1. **[NEEDS CLARIFICATION: define the automated screening rule set and its false-positive/false-negative handling; confirm this reduced (automation-only) safety net is acceptable given the constitution's safety-first principle, particularly for alcohol/age-restricted activities.]**
...
- **FR-026**: The system MUST display distance as a distance band (e.g., <1 km, 1–5 km, 5–15 km, 15+ km) rather than an exact distance. **[NEEDS CLARIFICATION: confirm final band thresholds, the location source/freshness used to compute the band, and the point in the journey at which distance is first shown.]**
...
- **FR-029**: A meeting qualifies as completed, and its participants become eligible for follow-up rating, when the associated selection was not cancelled by either party before the meetup (see FR-038). The system MUST send the rating and feedback prompt as an asynchronous follow-up, not instantly during or immediately after the meeting. The system MUST allow participants to provide public star ratings, written feedback, and photos about one another through that follow-up. A feedback photo that shows another identifiable person MUST NOT be made public without that person's explicit in-app consent captured before publication. **[NEEDS CLARIFICATION: define the follow-up prompt's timing/delay, rating scale, whether feedback is mutual/one-way, the consent-capture flow and revocation, display rules, moderation, reporting, removal, and appeals.]**
- **FR-030**: A user without an active eligible subscription MUST be allowed to broadcast no more than three offers per calendar month. The applicable calendar month MUST be determined from the city in the user's registered address. **[NEEDS CLARIFICATION: define how the registered-address city maps to a calendar/time zone, address-change timing, whether stopped/expired/rebroadcast offers count, and how abuse or system failures affect the count.]**
...
- **City Interest**: A city a user has deliberately registered as relevant for offers; a user may have more than one. Eligibility for a specific offer requires both the registered city interest and being within a configurable distance/radius of that offer's location (see FR-004).
- **Current Location**: The user's present location, sampled via device GPS periodically while the app is open and used to prioritize relevant offers when available and permitted; falls back to the user's last known location when live GPS is unavailable. Its precision threshold, storage duration, and visibility are unresolved.
- **Meet Offer**: A time-bound invitation created by one user for an activity at a specific, selected, live, or moving place. It has an activity, place, city association, creator, a creator-selected lifetime (5–30 minutes, default 15, counted from publish confirmation), a recipient capacity fixed at publish time (cannot change while active — see FR-008), status (active/stopped/expired), and expiry.
...
- **SC-002**: In acceptance testing, a recipient with the offer city registered as an interest and located within the eligibility radius is selected as eligible for that offer, while a recipient missing either condition is not selected unless a future approved matching rule says otherwise.
...
3. The exact place is revealed only after creator selection, and is captured as a single snapshot at publish time with no further updates afterward — even for a moving/transient place (see Clarifications, FR-002). Still open: the required place fields for a moving/transient context such as a queue.
4. Current location is sampled via periodic device GPS while the app is open, falling back to last known location when live GPS is unavailable (see Clarifications). Still open: the "nearby" radius/threshold, the GPS sampling interval, permission-denied behavior, how long a last-known location stays valid before being treated as stale, and the user-visible explanation of this behavior.
5. A recipient must have both the city registered as an interest and be within a distance/radius of the offer (see Clarifications). Still open: the default/configurable radius value(s), and whether availability, language, activity-preference, or an additional safety filter also applies.
6. Capacity is fixed at publish time (1–10), cannot be changed while the offer is active, and a selected recipient cannot be swapped for another (see Clarifications, FR-008). Still open: how a non-selection is communicated when some, but not all, interested recipients are chosen. (Selected recipients share one group chat with the creator — see Clarifications.)
...
8. Distance is shown as a band (e.g., <1km, 1–5km, 5–15km, 15km+), and an unverified user cannot hide the absent verification badge (see Clarifications, FR-014, FR-026). Still open: final band thresholds, the location source/freshness used to compute the band, and other photo/profile visibility controls.
9. A block request is routed to moderation review before it takes effect broadly, but the submitting user's own view updates immediately so they stop seeing the blocked user (see Clarifications, FR-015). Still open: review turnaround time, and the exact v1 behaviors for text/media chat, reporting review, emergency guidance, trusted-contact sharing, ratings/reviews, and data retention.
10. Prohibited/high-risk activities (paid meetings, private homes, dating/romantic framing) are screened via automated keyword/content screening at publish time only; v1 does not require human moderation review before publish (see Clarifications, FR-039). This is a notable reduction in the safety net relative to the constitution's safety-first principle and is flagged for explicit product-owner sign-off. Still open: the screening rule set, its false-positive/negative handling, whether alcohol/smoking/travel-boat activities are blocked automatically or only handled via post-publish reporting, and appeal rules for a falsely blocked offer.
11. Push notification (targeting ~30-second delivery) plus the in-app live feed as a fallback are required (see Clarifications, FR-006). Still open: behavior when push permission is denied, and how the delivery-time target is measured/alerted on in production.
...
15. A meeting counts as completed (rating-eligible) unless either party cancelled beforehand, and the rating prompt is sent as an asynchronous follow-up rather than instantly (see Clarifications, FR-029). Still open: the follow-up's exact timing/delay, whether ratings must be mutual or can be one-way, what star scale is used, and when a rating becomes public.
...
18. The free-offer calendar month is determined by the user's registered-address city. How is that city mapped to an authoritative time zone, what happens when the registered address changes, and do stopped, expired, or rebroadcast offers count as a broadcast?
