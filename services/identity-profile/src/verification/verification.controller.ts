...
import { SignupAgeAssuranceDto, SubmitGovernmentIdDto, SubmitPhotoVerificationDto } from './dto';
...
export class VerificationController {
  ...
  @Post('photo')
  @Throttle({ default: { limit: 5, ttl: 60_000 } })
  async submitPhoto(@Body() dto: SubmitPhotoVerificationDto, @Req() req: Request) {
    const userId = (req as any).verifiedPrincipal?.userId;
    const correlationId = (req as any).correlationId ?? 'unknown';
    return this.verification.submitPhotoVerification(userId, dto, correlationId);
  }
}
